                    ...
                    <td>
                        <div class="d-flex gap-1 flex-wrap">
                            ...
                            @if(in_array($roleName, ['super_admin', 'event_manager']))
                                @if($sName === 'added')
                                <button class="btn btn-sm btn-outline-warning" wire:click="changeStatus({{ $event->id }}, 'under_review')" wire:confirm="بدء مراجعة هذه الفعالية؟" title="مراجعة">
                                    <i class="bi bi-search"></i> مراجعة
                                </button>
                                @endif
                            ...
                            <button class="btn btn-sm btn-outline-danger" wire:click="changeStatus({{ $event->id }}, 'cancelled')" wire:confirm="هل تريد إلغاء الفعالية؟" title="إلغاء">
